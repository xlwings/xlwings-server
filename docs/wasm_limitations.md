...
- Configuration from the `app/wasm/.env` file (not from the `.env` file though, which is the configuration for xlwings Server)

To see exactly what the user has access to, inspect the `dist` folder after running the `python run.py wasm` command.
