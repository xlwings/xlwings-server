...
By setting `XLWINGS_AUTH_PROVIDERS` in the [](server_config.md), xlwings Server will authenticate calls to [](custom_functions.md) and [](custom_scripts.md). Since the task pane is completely customizable, it is your responsibility to lock down the desired endpoints:
...
- You will need to provide the `Authorization` header with every request. For `htmx`, there is a sample included under [`xlwings_server/templates/examples/auth`](https://github.com/xlwings/xlwings-server/tree/v1/xlwings_server/templates/examples/auth).
