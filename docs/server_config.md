...
- `XLWINGS_OBJECT_CACHE_MAXSIZE`: capacity of the in-memory object cache, which is only used when `XLWINGS_OBJECT_CACHE_URL` is not configured (default: `1000`). Least-recently-used entries are evicted beyond it, so it limits how many live object handles can be held at once. Since superseded handles are deleted on recalculation, the cache holds roughly one object per handle cell; lower this if you work with very large objects.
