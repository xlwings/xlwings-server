...
- `XLWINGS_OBJECT_CACHE_MAXSIZE`: capacity of the in-memory object cache, which is only used when `XLWINGS_OBJECT_CACHE_URL` is not configured (default: `100`). Least-recently-used entries are evicted beyond it, so it implicitly limits how many live object handles a workbook can hold at once.
